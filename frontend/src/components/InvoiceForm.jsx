import React from "react";

const InvoiceForm = ({ form, onFormChange, total }) => {
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    onFormChange(name, value);
  };

  const handleFileChange = (e) => {
    const files = Array.from(e.target.files);

    if (e.target.name === "invoice_file") {
      onFormChange("invoice_file", files[0] || null);
    } else {
      onFormChange("supporting_documents", files);
    }
  };

  return (
    <form className="">
  <div className="gap-4 mb-8 text-sm">
    <div className="flex items-center mb-2">
      <label className="font-medium w-64">
        Invoice Type <span className="text-red-500">*</span>
      </label>
      <select
        className="border rounded px-2 py-1 w-[28%]"
        name="invoice_type"
        value={form.invoice_type}
        onChange={handleInputChange}
      >
        <option>Vendor Credit Note</option>
        <option>Vendor Debit Note</option>
        <option>Vendor FDA</option>
        <option>Vendor Invoice</option>
        <option>Vendor Proforma Invoice</option>
      </select>
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">Correspondance Proforma Invoice (If Any)</label>
      <input
        type="text"
        className="border rounded px-2 py-1 w-[28%]"
        name="corresponding_proforma_invoice"
        value={form.corresponding_proforma_invoice}
        onChange={handleInputChange}
      />
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">
        Invoice No. <span className="text-red-500">*</span>
      </label>
      <input
        type="text"
        className="border rounded px-2 py-1 w-[28%]"
        name="invoice_no"
        value={form.invoice_no}
        onChange={handleInputChange}
      />
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">
        Invoice Date <span className="text-red-500">*</span>
      </label>
      <input
        type="date"
        className="border rounded px-2 py-1 w-[28%]"
        name="invoice_date"
        value={form.invoice_date}
        onChange={handleInputChange}
      />
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">Invoice Due Date</label>
      <input
        type="date"
        className="border rounded px-2 py-1 w-[28%]"
        name="invoice_due_date"
        value={form.invoice_due_date}
        onChange={handleInputChange}
      />
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">Purchase Order No.</label>
      <input
        type="text"
        className="border rounded px-2 py-1 w-[28%]"
        name="purchase_order_no"
        value={form.purchase_order_no}
        onChange={handleInputChange}
      />
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">Purchase Order Date</label>
      <input
        type="date"
        className="border rounded px-2 py-1 w-[28%]"
        name="received_date"
        value={form.received_date}
        onChange={handleInputChange}
      />
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">Port</label>
      <select
        className="border rounded px-2 py-1 w-[28%]"
        name="port"
        value={form.port}
        onChange={handleInputChange}
      >
        <option>Mumbai Port</option>
      </select>
    </div>

    <div className="flex items-center mb-2">
      <label className="font-medium w-64">Office/Vessel</label>
      <select
        className="border rounded px-2 py-1 w-[28%]"
        name="office_vessel"
        value={form.office_vessel}
        onChange={handleInputChange}
      >
        <option>Willows (vessel)</option>
      </select>
    </div>
  </div>

  <div className="flex items-center mb-2">
    <label className="font-medium w-64">
      Total Amount <span className="text-red-500">*</span>
    </label>
    <div className=" mr-2 w-[28%] bg-gray-200 px-3 py-1 rounded">{total}</div>
  </div>

  <div className="mb-6 mt-4">
    <label className="font-medium block mb-2">
      Attachments (PDF, JPG, JPEG)<span className="text-red-500">*</span>
    </label>
    <input
      type="file"
      name="invoice_file"
      onChange={handleFileChange}
      accept=".pdf, .jpg, .jpeg, image/jpeg, application/pdf"
      className="block w-full border rounded px-2 py-2 file:mr-4 file:py-1 file:px-4 file:rounded file:border-0 file:bg-blue-600 file:text-white hover:file:bg-blue-700"
    />
    <input
      type="file"
      name="supporting_documents"
      onChange={handleFileChange}
      accept=".pdf, .jpg, .jpeg, image/jpeg, application/pdf"
      multiple
      className="block w-full border rounded px-2 py-2 mt-2 file:mr-4 file:py-1 file:px-4 file:rounded file:border-0 file:bg-blue-600 file:text-white hover:file:bg-blue-700"
    />

    {form.supporting_documents?.length > 0 && (
      <div className="mt-2 text-sm text-gray-700">
        <strong>{form.supporting_documents.length}</strong> file
        {form.supporting_documents.length > 1 ? "s" : ""} selected
      </div>
    )}

    <ul className="mt-2 list-disc pl-5 text-sm text-gray-700">
      {form.supporting_documents?.map((file, idx) => (
        <li key={idx}>
          {file instanceof File ? file.name : file}
        </li>
      ))}
    </ul>
  </div>
</form>

  );
};

export default InvoiceForm;